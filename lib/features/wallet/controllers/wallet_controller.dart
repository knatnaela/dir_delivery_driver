...
import 'package:dir_delivery_driver/features/trip/domain/models/trip_model.dart';
import 'package:dir_delivery_driver/features/wallet/domain/models/payment_gateway_model.dart';
import 'package:dir_delivery_driver/features/wallet/domain/models/pending_settled_withdraw_model.dart';
import 'package:dir_delivery_driver/features/wallet/domain/models/withdraw_method_info_model.dart';
import 'package:dir_delivery_driver/features/wallet/domain/services/wallet_service_interface.dart';
import 'package:dir_delivery_driver/features/wallet/widgets/delete_confermation_dialog_widget.dart';
import 'package:dir_delivery_driver/features/wallet/widgets/withdraw_successful_dialog_widget.dart';
...
import 'package:dir_delivery_driver/features/wallet/domain/models/loyalty_point_model.dart';
...
class WalletController extends GetxController implements GetxService {
  ...
  Future<void> getWithdrawMethodInfoList(int offset) async {
    ...
    if (response.statusCode == 200) {
      if (offset == 1) {
        ...
        withdrawMethodInfoData?.totalSize = WithdrawMethodInfoData.fromJson(response.body).totalSize;
        withdrawMethodInfoData?.offset = WithdrawMethodInfoData.fromJson(response.body).offset;
        withdrawMethodInfoData?.data?.addAll(WithdrawMethodInfoData.fromJson(response.body).data!);
      }
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  Future<void> createWithdrawMethodInfo(String methodName) async {
    ...
    Response response =
        await walletServiceInterface.createWithdrawMethodInfo(keyList, inputValueList, selectedMethod!.id!);
    Get.back();
    if (response.statusCode == 200) {
  ...
  Future<void> updateWithdrawMethodInfo(String methodName, String methodInfoId, int methodId) async {
    ...
    Response response =
        await walletServiceInterface.updateWithdrawMethodInfo(keyList, inputValueList, methodId, methodInfoId);
    Get.back();
    if (response.statusCode == 200) {
