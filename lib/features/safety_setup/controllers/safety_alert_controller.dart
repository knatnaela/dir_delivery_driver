...
import 'package:dir_delivery_driver/features/safety_setup/domain/models/customer_alert_details.dart';
import 'package:dir_delivery_driver/features/safety_setup/domain/models/other_emergency_number_model.dart';
import 'package:dir_delivery_driver/features/safety_setup/domain/models/precaution_list_model.dart';
import 'package:dir_delivery_driver/features/safety_setup/domain/models/safety_alert_reason_model.dart';
import 'package:dir_delivery_driver/features/safety_setup/domain/services/safety_alert_service_interface.dart';
import 'package:dir_delivery_driver/features/safety_setup/widgets/safety_alert_delay_widget.dart';
...
class SafetyAlertController extends GetxController implements GetxService {
  ...
  void checkDriverNeedSafety() async {
    oldPosition =
        await Geolocator.getCurrentPosition(locationSettings: LocationSettings(accuracy: LocationAccuracy.low));

    _timer?.cancel();

    _timer = Timer.periodic(
        Duration(seconds: Get.find<SplashController>().config?.safetyFeatureMinimumTripDelayTime ?? 60), (_) async {
      newLocation =
          await Geolocator.getCurrentPosition(locationSettings: LocationSettings(accuracy: LocationAccuracy.low));

      double distance = Geolocator.distanceBetween(oldPosition?.latitude ?? 0, oldPosition?.longitude ?? 0,
      ...
      if (distance > 20) {
        ...
        if (!(Get.isBottomSheetOpen ?? false) &&
            !((Get.find<RideController>().remainingDistanceItem?[0].durationSec ?? 0) <
                (Get.find<RideController>().remainingDistanceItem?[0].durationInTrafficSec ?? 0))) {
          Get.bottomSheet(
