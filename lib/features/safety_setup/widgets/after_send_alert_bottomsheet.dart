...
import 'package:dir_delivery_driver/features/safety_setup/widgets/other_emergency_number_widget.dart';
        ...
        child: Padding(
          ...
          child: Column(spacing: Dimensions.paddingSizeSmall, mainAxisSize: MainAxisSize.min, children: [
                ...
                itemBuilder: (context, index) {
                  return Container(
                    ...
                    decoration: BoxDecoration(
                        color: Theme.of(context).hintColor.withValues(alpha: 0.1),
                        borderRadius: BorderRadius.circular(Dimensions.paddingSizeSmall)),
                    child: Text('${safetyAlertController.driverSafetyDetails?.data?.reason?[index]}',
            ...
            if (Get.find<SplashController>().config?.safetyFeatureEmergencyGovtNumber != null)
              InkWell(
                onTap: () => launchUrl(
                    Uri.parse("tel: ${Get.find<SplashController>().config?.safetyFeatureEmergencyGovtNumber}")),
                child: Row(mainAxisSize: MainAxisSize.min, spacing: Dimensions.paddingSizeExtraSmall, children: [
                  Image.asset(Images.customerCall,
                      height: 18, width: 18, color: Theme.of(context).colorScheme.surfaceContainer),
                  Text(
            ...
            GetBuilder<SafetyAlertController>(builder: (safetyAlertController) {
              return safetyAlertController.otherEmergencyNumberModel?.data != null &&
                      (safetyAlertController.otherEmergencyNumberModel?.data?.isNotEmpty ?? false)
                  ? InkWell(
                      onTap: () {
                        Get.back();
                        Get.bottomSheet(
                                ...
                                child: InkWell(
                                  ...
                                  child: Container(
                                    ...
                                    child: Image.asset(Images.crossIcon,
                                        height: 10, width: 10, color: Get.isDarkMode ? Colors.white : null),
                                  ),
                                ),
                              ),
