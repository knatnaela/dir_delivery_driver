...
class VerificationScreen extends StatefulWidget {
  ...
  const VerificationScreen(
      {super.key, required this.number, required this.form, this.session, required this.countryCode});
            ...
            child: GetBuilder<AuthController>(builder: (authController) {
              return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (errorText.isNotEmpty)
                  Center(
                      child: Text(errorText, style: textRegular.copyWith(color: Theme.of(context).colorScheme.error))),
                !authController.isLoading
                    ? Padding(
                        ...
                        child: ButtonWidget(
                          ...
                          radius: 50,
                          textColor: authController.verificationCode.length == 6
                        ...
                        ? TextButton(
                            onPressed: () async {
                              if (Get.find<SplashController>().config?.isFirebaseOtpVerification ?? false) {
                                ...
                                authController
                                    .sendOtp(countryCode: widget.countryCode, number: widget.number)
                                    .then((value) {
                                  if (value.statusCode == 200) {
                                    _startTimer();
                                  }
                            ...
                            child: Text('resend_code'.tr,
                                style:
                                    textBold.copyWith(color: Theme.of(context).primaryColorDark.withValues(alpha: .6)),
