...
import 'package:dir_delivery_driver/features/chat/domain/models/channel_model.dart';
...
class ChatController extends GetxController implements GetxService {
  ...
  Future<void> createChannel(String userId, {required tripId}) async {
    ...
    if (response.statusCode == 200) {
      ...
      Get.to(() => MessageScreen(
          channelId: channelId,
          tripId: tripId,
          userName: '${map['data']['user']['first_name']} ${map['data']['user']['last_name']}'));
    } else {
      isLoading = false;
      ApiChecker.checkApi(response);
    }
  ...
  Future<void> sendMessage(String channelId, String tripId) async {
    ...
    Response response = await chatServiceInterface.sendMessage(
        conversationController.value.text, channelId, tripId, _selectedImageList, objFile);
    if (response.statusCode == 200) {
      isSending = false;
  ...
  void subscribeMessageChannel(String tripId) {
    ...
    if (Get.find<SplashController>().pusherConnectionStatus != null ||
      ...
      channel = PusherHelper.pusherClient!.privateChannel("private-driver-ride-chat.$id",
