...
import 'package:dir_delivery_driver/features/trip/domain/models/trip_cancellation_cause_list_model.dart';
...
import 'package:dir_delivery_driver/features/trip/domain/models/trip_overview_model.dart';
...
class TripController extends GetxController implements GetxService {
  ...
  Future<Response> getTripList(
      int offset, String from, String to, String tripType, String filter, String status) async {
    isLoading = true;
  ...
  Future<void> getTripOverView(String filter) async {
    ...
    if (response.statusCode == 200) {
      ...
      weekList.insert(
          1,
          double.parse((isToday ? tripOverView?.incomeStat?.sixAm ?? 0 : tripOverView?.incomeStat?.sun ?? 0)
              .toStringAsFixed(2)));
      weekList.insert(
          2,
          double.parse((isToday ? tripOverView?.incomeStat?.temAM ?? 0 : tripOverView?.incomeStat?.mon ?? 0)
              .toStringAsFixed(2)));
      weekList.insert(
          3,
          double.parse((isToday ? tripOverView?.incomeStat?.twoPm ?? 0 : tripOverView?.incomeStat?.tues ?? 0)
              .toStringAsFixed(2)));
      weekList.insert(
          4,
          double.parse((isToday ? tripOverView?.incomeStat?.sixPm ?? 0 : tripOverView?.incomeStat?.wed ?? 0)
              .toStringAsFixed(2)));
      weekList.insert(
          5,
          double.parse((isToday ? tripOverView?.incomeStat?.temPm ?? 0 : tripOverView?.incomeStat?.thu ?? 0)
              .toStringAsFixed(2)));
      weekList.insert(
          6,
          double.parse((isToday ? tripOverView?.incomeStat?.twoAm ?? 0 : tripOverView?.incomeStat?.fri ?? 0)
              .toStringAsFixed(2)));
      if (!isToday) {
