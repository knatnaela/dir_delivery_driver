...
class ZoomDrawer extends StatefulWidget {
  ...
  static ZoomDrawerState? of(BuildContext context) {
    return context.findAncestorStateOfType<State<ZoomDrawer>>() as ZoomDrawerState?;
  }
}

class ZoomDrawerState extends State<ZoomDrawer> with SingleTickerProviderStateMixin {
  ...
  void _onHorizontalDragStart(DragStartDetails startDetails) {
    final maxDragSlide = widget.isRtl ? context._screenWidth - widget.dragOffset : widget.dragOffset;

    final toggleValue = widget.isRtl ? _animationController.isCompleted : _animationController.isDismissed;

    final isDraggingFromLeft = toggleValue && startDetails.globalPosition.dx < maxDragSlide;

    final isDraggingFromRight = !toggleValue && startDetails.globalPosition.dx > maxDragSlide;

    _shouldDrag = isDraggingFromLeft || isDraggingFromRight;
  }

  void _onHorizontalDragUpdate(DragUpdateDetails updateDetails) {
    if (_shouldDrag == false && ![DrawerState.opening, DrawerState.closing].contains(_stateNotifier.value)) {
      return;
    }

    final dragSensitivity =
        drawerLastAction == DrawerLastAction.open ? widget.closeDragSensitivity : widget.openDragSensitivity;
  ...
  void _onHorizontalDragEnd(DragEndDetails dragEndDetails) {
    ...
    if (willFling) {
      final visualVelocityInPx = dragEndDetails.velocity.pixelsPerSecond.dx / (context._screenWidth * 50);
  ...
  void toggle({bool forceToggle = false}) {
    if (stateNotifier.value == DrawerState.open || (forceToggle && drawerLastAction == DrawerLastAction.open)) {
      close();
  ...
  void _animationStatusListener(AnimationStatus status) {
    switch (status) {
      case AnimationStatus.forward:
        if (drawerLastAction == DrawerLastAction.open && _animationController.value < 1) {
      ...
      case AnimationStatus.reverse:
        if (drawerLastAction == DrawerLastAction.closed && _animationController.value > 0) {
          _stateNotifier.value = DrawerState.opening;
        } else {
          _stateNotifier.value = DrawerState.closing;
        }
    ...
    Widget menuScreen = GestureDetector(
      ...
      child: SizedBox(
        ...
        child: Align(
          ...
          child: SizedBox(
            width: widget.menuScreenWidth ?? widget.slideWidth - (context._screenWidth / widget.slideWidth) + 150,
  ...
  Widget get mainScreenWidget {
    ...
    if (widget.shrinkMainScreen) {
      final mainSize = context._screenWidth - (widget.slideWidth * animationValue);
      mainScreen = SizedBox(
        width: mainSize,
        child: mainScreen,
      );
    ...
    if (widget.angle != 0 && widget.style != DrawerStyle.defaultStyle) {
      final rotationAngle = (((widget.angle) * pi * _slideDirection) / 180) * animationValue;
      ...
      children: [
        ...
        if (widget.showShadow) ...[
          ...
          AnimatedBuilder(
            ...
            child: Container(
              color: widget.shadowLayer1Color ?? widget.drawerShadowsBackgroundColor.withAlpha(60),
            ),
          ),
          ...
          AnimatedBuilder(
            ...
            child: Container(
              color: widget.shadowLayer2Color ?? widget.drawerShadowsBackgroundColor.withAlpha(180),
            ),
          )
        ],
  ...
  Widget _renderStyle3() {
    return AnimatedBuilder(
      animation: _animationController,
      builder: (_, __) {
        final xPosition = (widget.slideWidth / 2) * animationValue * _slideDirection;
        final scalePercentage = 1 - (animationValue * widget.mainScreenScale);
  ...
  Widget _renderStyle4() {
    return AnimatedBuilder(
      animation: _animationController,
      builder: (_, __) {
        final xPosition = (widget.slideWidth * 1.2) * animationValue * _slideDirection;
        final scalePercentage = 1 - (animationValue * widget.mainScreenScale);
        final yAngle = animationValue * (pi / 4) * _slideDirection;
